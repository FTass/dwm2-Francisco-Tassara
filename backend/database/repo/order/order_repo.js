// database/repo/order/order_repo.js
...
class OrderRepository {
  ...
  async findByNumber(orderNumber, options = {}) {
    const { populate = [], select = null, lean = true } = options;
    let q = Order.findOne({ orderNumber });
    if (select) q = q.select(select);
    if (populate?.length) q = q.populate(populate);
    if (lean) q = q.lean();
    return await q;
  ...
  async updateById(orderId, input) {
    ...
    return await Order.findByIdAndUpdate(orderId, input, {
      new: true,
      runValidators: true,
      context: 'query',
    });
